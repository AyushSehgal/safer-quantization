...
class MyTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if loss_type == "cal":
            ...
            # 2. IMMEDIATE SANITIZATION (The Shield)
            # Cast to float32 and replace any NaNs/Infs generated by bfloat16 overflows
            # or CPU offloading with neutral, finite values to prevent gradient poisoning.
            ft_logits = torch.nan_to_num(ft_logits.float(), nan=1e-4, posinf=100.0, neginf=-100.0)
            q_logits  = torch.nan_to_num(q_logits.float(), nan=1e-4, posinf=100.0, neginf=-100.0)
            pt_logits = torch.nan_to_num(pt_logits.to(ft_logits.device).float(), nan=1e-4, posinf=100.0, neginf=-100.0)

            # 3. Target Probabilities (Explicitly detached)
            with torch.no_grad():
            ...
            # 5. Quantized Log-Probs (Safely bounded)
