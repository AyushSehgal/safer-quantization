...
class MyTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if loss_type == "cal":
            ...
            ft_logits = self.get_ori_outputs(model, inputs).logits.detach()
            outputs = model(**inputs)
            q_logits = outputs.logits
            pt_logits = self.get_pretrained_outputs(inputs).logits.detach()

            # Cast and sanitize (maintaining gradients for q_logits)
            ft_logits = torch.nan_to_num(ft_logits.float(), nan=1e-4)
            q_float   = torch.nan_to_num(q_logits.float(), nan=1e-4)
            pt_logits = torch.nan_to_num(pt_logits.to(ft_logits.device).float(), nan=1e-4)
            ...
            log_q = F.log_softmax(q_float, dim=-1)
            log_q_clamped = log_q.clamp(min=-20.0, max=0.0)

            p_ft_stop = p_ft.gather(-1, s_top)
            log_p_ft_stop = log_p_ft.gather(-1, s_top)
            log_q_stop = log_q_clamped.gather(-1, s_top)

            p_pt_sdiff = p_pt.gather(-1, s_diff)
            log_p_pt_sdiff = log_p_pt.gather(-1, s_diff)
            log_q_sdiff = log_q_clamped.gather(-1, s_diff)
            ...
            # These will print during the backward pass (before the optimizer steps)
            if loss.requires_grad:
                loss.register_hook(lambda g: print(f"[HOOK 1] Loss Scalar Grad NaN: {torch.isnan(g).any().item()}"))
            if log_q.requires_grad:
                log_q.register_hook(lambda g: print(f"[HOOK 2] log_q Grad NaN: {torch.isnan(g).any().item()}"))
            if q_logits.requires_grad:
                q_logits.register_hook(lambda g: print(f"[HOOK 3] q_logits Grad NaN (Entering Model): {torch.isnan(g).any().item()}"))
