...
def load_safetybench(language: str = "en"):
    ...
    try:
        dataset = load_dataset("thu-coai/SafetyBench", language, split="test")
    except Exception:
        # Fallback: try loading without language config
        try:
            dataset = load_dataset("thu-coai/SafetyBench", split="test")
            if language == "en":
                dataset = dataset.filter(lambda x: x.get("language", "en") == "en")
            elif language == "zh":
                dataset = dataset.filter(lambda x: x.get("language", "zh") == "zh")
